...
export function gonghap(birthA, birthB, lunA, lunB) {
  ...
  const notes = [];
  if (ab.hits.length) notes.push(`그대 부처궁의 ${ab.hits.join("·")}성이 이 사람의 명궁에 그대로 떠 있네`);
  if (ba.hits.length) notes.push(`상대 부처궁의 ${ba.hits.join("·")}성이 그대의 명궁에 그대로 떠 있네`);
  if (samhap) notes.push(`두 사람의 명궁 자리(${JI[posA]}·${JI[posB]})가 삼합으로 묶여 있네`);
...
export function gonghapDetail(g) {
  ...
  if (g.ab.hits.length) {
    paras.push(`그대의 부처궁에는 ${g.aSpouse.join("·")}이 앉아 "${love(g.aSpouse)} 사랑"을 그려왔는데, 이 사람의 명궁에 바로 그 ${g.ab.hits.join("·")}이 떠 있어요. 그대가 마음 깊이 원하던 결을 타고난 사람이라는 뜻이에요.`);
  } else if (mySp && bMain) {
    paras.push(`그대의 부처궁 ${mySp}은 ${love(g.aSpouse)} 사랑을 원해요. 이 사람의 명궁 ${bMain}은 ${look(g.bSelf)} 성정이라, ${g.ab.grpMatch ? "결이 같은 계열로 흐르니 처음부터 낯설지 않을 거예요" : "결은 다르지만 그만큼 그대에게 없는 것을 채워주는 배치예요"}.`);
  }

  // ③ 상대의 이상형 ↔ 나
  const bSp = g.bSpouse[0];
  if (g.ba.hits.length) {
    paras.push(`반대로 이 사람의 부처궁 ${g.bSpouse.join("·")}이 그대의 명궁과 겹칩니다 — 이 사람이 그려온 짝의 모습에 그대가 닿아 있어요. 서로가 서로의 그림 안에 있는 셈이지요.`);
  } else if (bSp) {
    paras.push(`이 사람의 부처궁에는 ${bSp}이 있어 ${love(g.bSpouse)} 관계를 바라는 사람이에요. 그대의 결을 천천히 보여주면 마음이 기우는 구조예요.`);
  }

  // ④ 삼합 — 생활 리듬
  if (g.samhap) {
    paras.push(`두 사람의 명궁 자리(${g.jiA}·${g.jiB})는 삼합으로 묶여 있어요. 만나는 시간대, 쉬는 방식, 결정의 속도 같은 생활의 박자가 잘 맞는 편이라 오래 볼수록 편해지는 연이에요.`);
  }
