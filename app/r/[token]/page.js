  ...
  return text.split(/\n{2,}/).map((para, i) => (
    <p key={i} style={{ fontSize: 15.5, marginBottom: 14, lineHeight: 1.9 }}>
      {para.split(/(\*\*[^*]+\*\*)/g).map((seg, j) =>
        seg.startsWith("**") && seg.endsWith("**") ? <b key={j} style={{ color: "var(--seal)" }}>{seg.slice(2, -2)}</b> : <span key={j}>{seg}</span>
      )}
    </p>
  ));
}
  ...
  if (!lead || !lead.report) {
    return (
      <main className="wrap" style={{ paddingTop: 80, textAlign: "center" }}>
        ...
        <h1 className="display" style={{ fontSize: 24, margin: "16px 0 10px", color: "var(--ink)" }}>리포트를 찾을 수 없습니다</h1>
  ...
  return (
    <main className="wrap" style={{ paddingTop: 40, paddingBottom: 60 }}>
      <div style={{ textAlign: "center", marginBottom: 30 }}>
        <div className="eyebrow">{CONFIG.BRAND_HANJA} · 진로·학업 종합 리포트</div>
        ...
        return (
          <div className="card" key={s.id} style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 12 }}>
              ...
              <h2 className="display" style={{ fontSize: 18, color: "var(--ink)" }}>{s.title}</h2>
            </div>
            {renderText(text)}
          </div>
        );
      ...
      <footer style={{ paddingTop: 30 }}>
        <p>
          ...
          교육적·의학적 판단의 유일한 근거가 될 수 없습니다.<br /><br />
