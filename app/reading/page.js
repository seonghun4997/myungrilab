...
function captureUtm() {
  try {
    const saved = localStorage.getItem("hs_utm");
    ...
    if (Object.keys(utm).length) localStorage.setItem("hs_utm", JSON.stringify(utm));
    return utm;
  } catch (e) { return {}; }
}
...
export default function Home() {
  ...
  useEffect(() => {
    captureUtm();
    try {
      const r = localStorage.getItem("hs_resume");
      if (r) {
        const snap = JSON.parse(r);
        if (snap.leadId && Date.now() - (snap.ts || 0) < 48 * 3600 * 1000) {
          setPaySnap(snap);
          // 토스 앱에 다녀오다 탭이 리셋된 경우 — 랜딩 없이 결제 화면으로 바로 복원
          const pend = Number(localStorage.getItem("hs_paypending") || 0);
          if (pend && Date.now() - pend < 2 * 3600 * 1000) {
            localStorage.removeItem("hs_paypending");
            resumePayWith(snap);
          }
        }
      }
      const s = localStorage.getItem("hs_last");
      if (s) setLastSnap(JSON.parse(s));
      // 허브 타일에서 넘어온 관심사(focus) — 리드에 저장해 관심사 데이터 수집
      try {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem("hs_form", JSON.stringify(rest));
    } catch (e) {}
  }, [form]);
  ...
  const resumePayWith = async (snap) => {
    if (!snap) return;
    try {
      const mod = await import("korean-lunar-calendar");
      const KLC = mod.default || mod;
      const c = new KLC();
      if (!c.setSolarDate(snap.sy, snap.sm, snap.sd)) throw new Error("date");
      const lun = c.getLunarCalendar();
      const z = computeZiwei({
        lunarYear: lun.year, lunarMonth: lun.month, lunarDay: lun.day,
        hourBranch: snap.slot ?? 6, gender: snap.gender,
        solarYear: snap.sy, solarMonth: snap.sm, solarDay: snap.sd,
      });
      setZiwei(z);
      setForm((f) => ({ ...f, y: snap.sy, m: snap.sm, d: snap.sd, slot: snap.slot ?? 6, gender: snap.gender, name: snap.name || f.name, cal: "solar" }));
      setLeadId(snap.leadId);
      setLeadToken(snap.leadToken);
      ev("resume_pay");
      goto("pay");
    } catch (e) {}
  };
  const resumePay = () => resumePayWith(paySnap);
  ...
  const submitAll = async () => {
    ...
    try {
      localStorage.setItem("hs_last", JSON.stringify({ y: sy, m: sm, d: sd, slot: form.slot, gender: form.gender, name: form.name.trim() }));
    } catch (e) {}
    ...
    try {
      ...
      const res = await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          birth: {
            ...
            loveStatus: form.loveStatus || null,
            focus: (typeof window !== "undefined" && sessionStorage.getItem("hs_focus")) || null,
            utm: captureUtm(),
          },
          salNames: [
      ...
      try {
        const s = JSON.parse(localStorage.getItem("hs_last") || "{}");
        s.token = data.token || null;
        localStorage.setItem("hs_last", JSON.stringify(s));
        // 결제 복구용 스냅샷 — 페이지가 날아가도 '이어서 결제'로 되살린다
        localStorage.setItem("hs_resume", JSON.stringify({
...
const LOVE_STATUS = ["솔로", "썸", "연애 중", "이별 후", "재회 고민"];
      ...
      <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 14 }}>
        {LOVE_STATUS.map((st) => (
          <button
            key={st}
            type="button"
            onClick={() => { setForm({ ...form, loveStatus: form.loveStatus === st ? "" : st }); ev("love_status", { st }); }}
            style={{
              padding: "8px 14px", fontSize: 13.5, borderRadius: 20, cursor: "pointer",
              border: form.loveStatus === st ? "1px solid var(--gold)" : "1px solid rgba(196,176,255,.35)",
              background: form.loveStatus === st ? "rgba(255,212,121,.14)" : "rgba(139,108,255,.06)",
              color: form.loveStatus === st ? "var(--gold)" : "var(--tx)",
              fontWeight: form.loveStatus === st ? 700 : 400,
            }}
          >
            {st}
          </button>
...
function useCountdown() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 만료됐거나 없으면 새 마감으로 재장전 — 재방문자가 00:00:00에 멈춘 타이머를 보는 버그 방지
      if (!deadline || deadline - Date.now() <= 0) {
        ...
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <a
            ...
            onClick={(e) => {
              ...
              try { localStorage.setItem("hs_paypending", String(Date.now())); } catch (err) {} // 앱에서 돌아올 때 자동 복원용 깃발
