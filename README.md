# 명문서당 (名門書堂) — 자녀사주 전문관 디비 퍼널

"아이 인생의 정답지는 사주에 있습니다" — 대한민국 1등 자녀사주 전문관

퍼널: 영상 랜딩("학원비 쓰기 전에 이것부터") → 훈장님 문답(디비 입력) → 재능 등급 진단(무료) + 공부살 잠금 → 결제 → 관리자가 리포트 생성 → 카카오톡 전달

확장 로드맵: 부모-자녀 궁합 → 형제 사주 → 수능 해 정밀 → (별도 라인) 살풀이 호러 퍼널 재활용 가능 (git 이력에 v6 보존)
