...
export function computeSaju(input) {
  ...
  for (const { k, ji } of allJi) {
    if (ji === MUNCHANG[dayGan] && !sinsal.find((s) => s.name === "문창귀인")) sinsal.push({ name: "문창귀인", pos: k });
  }
  // 괴강/백호 (일주)
  ...
  if (YANGIN[dayGan] !== undefined) {
    for (const { k, ji } of allJi) {
      if (ji === YANGIN[dayGan] && !sinsal.find((s) => s.name === "양인" && s.pos === k)) sinsal.push({ name: "양인", pos: k });
    }
  }
  // 탕화 (일지 인/오/축)
  if ([2, 6, 1].includes(dayJi)) sinsal.push({ name: "탕화", pos: "day" });
  // 원진/귀문 (지지 쌍 — 어느 두 기둥이든)
  ...
  for (let i = 0; i < allJi.length; i++) {
    for (let j = i + 1; j < allJi.length; j++) {
      ...
      if (WONJIN[a.ji] === b.ji && !sinsal.find((s) => s.name === "원진")) sinsal.push({ name: "원진", pos: a.k });
      if (GWIMUN[a.ji] === b.ji && !sinsal.find((s) => s.name === "귀문")) sinsal.push({ name: "귀문", pos: a.k });
    }
  }

  // --- 삼재 (년지 삼합국 기준) ---
  // 신자진생→인묘진년 / 해묘미생→사오미년 / 인오술생→신유술년 / 사유축생→해자축년
  const SAMJAE_START = { 1: 2, 3: 5, 0: 8, 2: 11 }; // groupOf(년지) → 삼재 시작 지지
  const sjStart = SAMJAE_START[groupOf(yearJi)];
  const nowY = new Date().getFullYear();
  function samjaeYearsNear(refYear) {
    // refYear 이전·이후에서 가장 가까운 삼재 3년 세트의 시작 연도 찾기
    for (let yy = refYear; yy >= refYear - 12; yy--) {
      if ((((yy - 4) % 12) + 12) % 12 === sjStart) {
        if (yy + 2 >= refYear) return [yy, yy + 1, yy + 2]; // 진행 중이거나 미래
      }
    }
    for (let yy = refYear; yy <= refYear + 12; yy++) {
      if ((((yy - 4) % 12) + 12) % 12 === sjStart) return [yy, yy + 1, yy + 2];
    }
    return null;
  }
  function samjaePast(refYear) {
    for (let yy = refYear - 1; yy >= refYear - 13; yy--) {
      if ((((yy - 4) % 12) + 12) % 12 === sjStart && yy + 2 < refYear) return [yy, yy + 1, yy + 2];
    }
    return null;
  }
  const samjaeNext = samjaeYearsNear(nowY);
  const samjae = {
    inNow: samjaeNext ? samjaeNext[0] <= nowY && nowY <= samjaeNext[2] : false,
    next: samjaeNext,
    past: samjaePast(nowY),
  };
