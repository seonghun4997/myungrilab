...
import { iljuCharacter, bluntLine, buildTeaser, salList } from "./insights";

export const SECTIONS = [
  { id: "sal", hanja: "煞", title: "살 정밀 해부 — 검출된 살의 정체와 발동 조건" },
  { id: "overview", hanja: "總", title: "타고난 그릇 — 살을 품은 그릇" },
  { id: "wealth", hanja: "財", title: "재물 — 새는 구멍과 채우는 길" },
  { id: "love", hanja: "緣", title: "인연 — 붙는 살, 끊는 살" },
  { id: "years", hanja: "運", title: "향후 5년 — 발동 시간표" },
  { id: "daeun", hanja: "大", title: "다음 대운 — 판이 바뀌는 해" },
];
...
export function buildFacts(saju) {
  ...
  return {
    ...
    검출살: salList(saju).map((s) => ({ 이름: s.name, 한자: s.hanja, 별칭: s.short, 길흉: s.good ? "길신" : "흉살" })),
    삼재: saju.samjae
      ? {
          현재삼재중: saju.samjae.inNow,
          가까운삼재: saju.samjae.next ? `${saju.samjae.next[0]}~${saju.samjae.next[2]}년` : null,
          지난삼재: saju.samjae.past ? `${saju.samjae.past[0]}~${saju.samjae.past[2]}년` : null,
        }
      : null,
